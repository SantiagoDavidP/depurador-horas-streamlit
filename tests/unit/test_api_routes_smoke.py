...
@pytest.mark.unit
def test_api_route_batch_download_and_baninter_zip_flow(monkeypatch):
    ...
    monkeypatch.setattr(
        handlers_batch, "auto_detect_profile_from_files", lambda *_: ("cliente_bit", {"company": "NOVA"})
    )
    ...
    analyze_resp = client.post(
        ...
        files=[
            (
                "files",
                ("sample.xlsx", b"fake-xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"),
            )
        ],
    )
    ...
    assert process_json["results"][0]["summary"]["quality_score"] == 95.0
    ...
    download_resp = client.get(f"/api/download/{consolidate_json['download_id']}")
    assert download_resp.status_code == 200
    assert download_resp.content == b"consolidado-bytes"

    zip_resp = client.post("/api/batch/baninter-zip", json={"batchId": batch_id})
    assert zip_resp.status_code == 200
    zip_json = zip_resp.json()
    assert zip_json["filename"].endswith(".zip")

    zip_download = client.get(f"/api/download/{zip_json['download_id']}")
    assert zip_download.status_code == 200
    assert zip_download.content == b"zip-bytes"


@pytest.mark.unit
def test_api_route_individual_health_profile_me_and_docs_flow(monkeypatch):
    store = _MemoryFileStore()
    mapping = ColumnMapping(date="Fecha", hours="Horas", description="Actividad", project="Proyecto")
    profile = _profile()
    ...
    monkeypatch.setattr(handlers_individual_process, "is_baninter_result", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(handlers_individual_process, "_build_baninter_zip", lambda *_args, **_kwargs: b"baninter-unit")
    ...
    assert analyze_ind.json()["auto_profile_id"] == "cliente_bit"
    ...
    assert process_json["results"][0]["summary"]["quality_score"] == 95.0
    assert process_json["baninter_zip"]["filename"].endswith(".zip")

    result_download = client.get(f"/api/download/{process_json['results'][0]['download_id']}")
    assert result_download.status_code == 200
    assert result_download.content == b"excel-bytes"


@pytest.mark.unit
def test_app_openapi_contains_all_backend_routes():
    paths = set(app.openapi()["paths"].keys())
    assert "/api/batch/analyze" in paths
    assert "/api/batch/process" in paths
    assert "/api/batch/consolidate" in paths
    assert "/api/batch/baninter-zip" in paths
    assert "/api/download/{file_id}" in paths
    assert "/api/health" in paths
    assert "/api/individual/analyze" in paths
    assert "/api/individual/process" in paths
