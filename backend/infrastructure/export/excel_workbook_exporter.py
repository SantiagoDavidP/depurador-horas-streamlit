...
class ExcelWorkbookExporter:
    ...
    def export_workbook(
        ...
        corrected_df = to_pandas_table(corrected_df)
        errors_df = to_pandas_table(errors_df)
        if debug_hours_df is not None:
            debug_hours_df = to_pandas_table(debug_hours_df)
        if debug_detail_df is not None:
            debug_detail_df = to_pandas_table(debug_detail_df)
