...
def validate_bit_nova(
    ...
    # Validar tickets numéricos
    if ticket_col:
        invalid_tickets = pd.to_numeric(df[ticket_col], errors="coerce").isna()
        for idx in df[invalid_tickets].index:
            row_number = row_numbers[idx] if row_numbers and idx < len(row_numbers) else idx + 2
            issues.append(
                ValidationIssue(
                    fila=row_number,
                    fecha=str(df.at[idx, fecha_col]) if fecha_col else "",
                    tipo_error="ticket_no_numerico",
                    descripcion="El ticket debe ser numérico.",
