...
def validate_duplicate_entries(
    ...
    *,
    ...
    if perf:
        perf.add("validators.duplicate.normalize_desc", time.perf_counter() - t0)

    t0 = time.perf_counter()
    desc_counts = normalized_descriptions.value_counts()
    dup_descs = desc_counts[desc_counts >= min_duplicates].index
    index_map = normalized_descriptions.groupby(normalized_descriptions).indices

    for desc in dup_descs:
        if desc and len(desc) > 5:
            ...
            if (
                ...
                continue
            filas = list(index_map.get(desc, []))
            if not filas:
                continue
            if parsed_dates is None:
                fechas = df.loc[filas, date_column].apply(_normalize_date_output).tolist()
            else:
                fechas = parsed_dates.loc[filas].apply(_normalize_date_output).tolist()
    ...
    if perf:
        perf.add("validators.duplicate.exact", time.perf_counter() - t0)
    ...
    descriptions = normalized_descriptions.unique()

    # FIX PERFORMANCE: Limitar comparaciones O(n^2) a maximo 30 descripciones
    ...
    for i, desc1 in enumerate(descriptions):
        if not desc1 or len(desc1) < 10:
            continue
        # Solo comparar con proximos 10 items en lugar de todos los restantes
            ...
            if similarity >= similarity_threshold:
                ...
                if len(rows1) + len(rows2) >= min_duplicates:
                    ...
                    if (
                        ...
                        continue
                    if parsed_dates is None:
                        fechas = df.loc[rows1 + rows2, date_column].apply(_normalize_date_output).tolist()
                    else:
                    ...
                    issues.append(
                        ValidationIssue(
                            ...
                            valor_original=str(df.loc[idx_ref, description_column]),
                            valor_corregido="",
                        )
                    )
        if fuzzy_exceeded:
    ...
    if perf:
        perf.add("validators.duplicate.fuzzy", time.perf_counter() - fuzzy_start)
...
def validate_missing_fields(
    ...
    *,
    ...
    fechas = (
        df[date_column].apply(_normalize_date_output)
    ...
    for row_pos, col_pos in np.argwhere(empty_values):
        ...
        issues.append(
            ValidationIssue(
                ...
                descripcion=f"Campo vacío en columna '{col_name}'.",
                valor_original="",
                valor_corregido="",
            )
        )
    if perf:
        perf.add("validators.missing_fields.issues", time.perf_counter() - t0)
...
def run_all_validations(
    ...
    *,
    ...
    if numeric_hours is None or numeric_hours.isna().any():
        ...
        numeric_hours = _coerce_hours_series(df[hours_column])
        if perf:
            perf.add("validators.precompute.numeric_hours", time.perf_counter() - t0)
    ...
    if parsed_dates is None or getattr(parsed_dates, "isna", None) is None:
        ...
        if perf:
            perf.add("validators.precompute.parsed_dates", time.perf_counter() - t0)
    ...
    try:
        ...
        if perf:
            perf.add("validators.calendar", time.perf_counter() - t0)
    ...
    try:
        ...
        if perf:
            perf.add("validators.daily_hours", time.perf_counter() - t0)
    except Exception as exc:
    ...
    if description_column:
        try:
            ...
            if perf:
                perf.add("validators.description_quality", time.perf_counter() - t0)
            ...
            if len(df) > max_consecutive_identical:
                ...
                if perf:
                    perf.add("validators.copy_paste", time.perf_counter() - t0)
            else:
            ...
            if len(df) >= duplicate_min_occurrences:
                ...
                if perf:
                    perf.add("validators.duplicate_entries", time.perf_counter() - t0)
            else:
            ...
            if perf:
                perf.add("validators.reasonable_hours", time.perf_counter() - t0)
        except Exception as exc:
    ...
    if project_column:
        try:
            ...
            if perf:
                perf.add("validators.project_consistency", time.perf_counter() - t0)
        except Exception as exc:
    ...
    if ticket_column and ticket_column in df.columns:
        try:
            ...
            if perf:
                perf.add("validators.ticket_format", time.perf_counter() - t0)
        except Exception as exc:
    ...
    try:
        ...
        if perf:
            perf.add("validators.missing_fields", time.perf_counter() - t0)
    except Exception as exc:
    ...
    if check_completeness:
        try:
            ...
            if perf:
                perf.add("validators.working_days", time.perf_counter() - t0)
        except Exception as exc:
